...
**Version:** 1.2 — Multi-Format Specification (stack & design synced)  
**Date:** September 14, 2026  
**Status:** 🟡 Pending Client Sign-off  
**Companion docs:** `technology-stack.md` (authoritative stack), `ui-ux-plan.md` (design system), `application-flow.md` (flows)  
...
| **Reliability** | State persists continuously to IndexedDB (Dexie) with an offline sync queue (PWA service worker); zero data loss on browser refresh, tab closure, or connectivity loss. Per `technology-stack.md` §3.6. |
| **Design Aesthetics** | "Engineering Drawing" design system: Tailwind CSS v4 with custom design tokens (graphite/paper palette, IBM Plex Sans/Mono typography), colorblind-safe status indicators, and tablet-first responsive layout. See `ui-ux-plan.md`. |
